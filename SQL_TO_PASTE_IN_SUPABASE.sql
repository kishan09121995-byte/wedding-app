...
where not exists (select 1 from catering_items where meal_name = 'Mandap Refreshments');

insert into catering_items (function_id, meal_name, rate_per_plate, min_guarantee_pax, venue)
select id, 'Lunch', 683, 150, 'Kiara-3 Ballroom' from functions where name = 'F1 - Mandap Ceremony'
where not exists (select 1 from catering_items where meal_name = 'Lunch' and venue = 'Kiara-3 Ballroom');

insert into catering_items (function_id, meal_name, rate_per_plate, min_guarantee_pax, venue)
select id, 'High Tea', 368, 200, 'AMARA Lawn' from functions where name = 'F2 - Haldi / Carnival'
where not exists (select 1 from catering_items where meal_name = 'High Tea');

insert into catering_items (function_id, meal_name, rate_per_plate, min_guarantee_pax, venue)
select id, 'Dinner', 893, 550, 'Kiara Ballroom' from functions where name = 'F3 - Sangeet / Pre-Wedding Reception'
where not exists (select 1 from catering_items where meal_name = 'Dinner' and venue = 'Kiara Ballroom');

insert into catering_items (function_id, meal_name, rate_per_plate, min_guarantee_pax, is_manual, venue)
select id, 'Late Night Snacks', 400, 0, true, 'Kiara Ballroom' from functions where name = 'F3 - Sangeet / Pre-Wedding Reception'
where not exists (select 1 from catering_items where meal_name = 'Late Night Snacks');

insert into catering_items (function_id, meal_name, rate_per_plate, min_guarantee_pax, venue)
select id, 'Breakfast - In-House Free', 0, 0, 'Kesar' from functions where name = 'F4 - Wedding Day'
where not exists (select 1 from catering_items where meal_name = 'Breakfast - In-House Free');

insert into catering_items (function_id, meal_name, rate_per_plate, min_guarantee_pax, is_manual, venue)
select id, 'Breakfast - Additional', 400, 0, true, 'Kesar' from functions where name = 'F4 - Wedding Day'
where not exists (select 1 from catering_items where meal_name = 'Breakfast - Additional');

insert into catering_items (function_id, meal_name, rate_per_plate, min_guarantee_pax, venue)
select id, 'Wedding Lunch', 893, 500, 'Kiara Ballroom' from functions where name = 'F4 - Wedding Day'
where not exists (select 1 from catering_items where meal_name = 'Wedding Lunch' and min_guarantee_pax = 500);
