...
import { Trash2, Plus, CheckCircle, AlertCircle, FileText, Download } from 'lucide-react';
...
export default function MenuManagement() {
    ...
    const checkDuplicates = (itemName, category) => {
        return menus
    ...
    const toggleItemSelection = (menuId, itemIndex) => {
        setSelectedItems(prev => {
            const current = prev[menuId] || new Set();
    ...
    const getSelectedCount = (menuId, total) => {
        return selectedItems[menuId]?.size || 0;
    };
    const exportToExcel = () => {
        const functionMenus = menus.filter(m => m.function_id === selectedFunction);
        const exportData = functionMenus.flatMap(menu => menu.items?.map((item, idx) => ({
    ...
    return (_jsxs("div", { className: "space-y-6", children: [_jsx(Toaster, { position: "top-right" }), _jsxs("div", { className: "bg-white rounded-lg p-6 shadow-sm border border-gray-200", children: [_jsxs("div", { className: "flex items-center gap-3 mb-4", children: [_jsx(FileText, { className: "w-8 h-8 text-rose-gold" }), _jsx("h1", { className: "text-3xl font-bold text-mauve", children: "Menu Management" })] }), _jsx("p", { className: "text-gray-600", children: "Manage wedding menu items by function and category" })] }), _jsxs("div", { className: "bg-white rounded-lg p-6 shadow-sm border border-gray-200", children: [_jsx("h2", { className: "text-lg font-semibold text-gray-800 mb-4", children: "Select Function" }), _jsx("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-3", children: functions.map((func) => (_jsx("button", { onClick: () => setSelectedFunction(func.id), className: `p-4 rounded-lg font-medium transition ${selectedFunction === func.id
                                ...
                                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`, children: func.name || `Function ${func.id}` }, func.id))) })] }), _jsxs("div", { className: "flex gap-3", children: [_jsxs("button", { onClick: () => setShowAddMenu(!showAddMenu), className: "bg-rose-gold hover:bg-rose-gold/90 text-white font-medium py-3 px-6 rounded-lg flex items-center gap-2 transition", children: [_jsx(Plus, { className: "w-5 h-5" }), "Add Menu Items"] }), _jsxs("button", { onClick: exportToExcel, className: "bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-6 rounded-lg flex items-center gap-2 transition", children: [_jsx(Download, { className: "w-5 h-5" }), "Export to Excel"] })] }), showAddMenu && (_jsxs("div", { className: "bg-white rounded-lg p-6 shadow-sm border border-gray-200 space-y-4", children: [_jsx("h3", { className: "text-lg font-semibold text-gray-800", children: "Add Menu Items" }), _jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4", children: [_jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-2", children: "Category" }), _jsx("select", { value: menuCategory, onChange: (e) => setMenuCategory(e.target.value), className: "w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-gold outline-none", children: categories.map((cat) => (_jsx("option", { value: cat, children: cat }, cat))) })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-2", children: "Timing" }), _jsxs("select", { value: menuTiming, onChange: (e) => setMenuTiming(e.target.value), className: "w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-gold outline-none", children: [_jsx("option", { value: "", children: "Select Timing" }), timings.map((time) => (_jsx("option", { value: time, children: time }, time)))] })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-2", children: "General Manager" }), _jsx("input", { type: "text", value: gmName, onChange: (e) => setGmName(e.target.value), placeholder: "GM Name", className: "w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-gold outline-none" })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-2", children: "Items to Select" }), _jsx("input", { type: "number", value: quantity, onChange: (e) => setQuantity(parseInt(e.target.value) || 1), min: 1, max: 20, className: "w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-gold outline-none" })] })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-2", children: "Menu Items (one per line)" }), _jsx("textarea", { value: menuItems, onChange: (e) => setMenuItems(e.target.value), placeholder: "e.g.\nPaneer Tikka\nTandoori Chicken\nVegetable Biryani", className: "w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-gold outline-none", rows: 6 })] }), _jsxs("div", { className: "flex gap-3", children: [_jsx("button", { onClick: handleAddMenu, disabled: uploading, className: "bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-6 rounded-lg transition disabled:opacity-50", children: uploading ? 'Adding...' : 'Add Items' }), _jsx("button", { onClick: () => setShowAddMenu(false), className: "bg-gray-300 hover:bg-gray-400 text-gray-700 font-medium py-2 px-6 rounded-lg transition", children: "Cancel" })] })] })), _jsxs("div", { className: "bg-white rounded-lg p-6 shadow-sm border border-gray-200", children: [_jsxs("h2", { className: "text-lg font-semibold text-gray-800 mb-4", children: [currentFunction?.name || 'Function', " Menus"] }), functionMenus.length === 0 ? (_jsx("p", { className: "text-gray-600 text-center py-8", children: "No menus added yet" })) : (_jsx("div", { className: "space-y-4", children: functionMenus.map((menu) => (_jsxs("div", { className: "border border-gray-200 rounded-lg p-4 space-y-3", children: [_jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsxs("h3", { className: "text-lg font-semibold text-gray-800", children: [menu.category, " Menu"] }), menu.approved ? (_jsxs("span", { className: "flex items-center gap-1 text-green-600 text-sm font-medium", children: [_jsx(CheckCircle, { className: "w-4 h-4" }), "Approved"] })) : (_jsxs("span", { className: "flex items-center gap-1 text-amber-600 text-sm font-medium", children: [_jsx(AlertCircle, { className: "w-4 h-4" }), "Pending"] }))] }), _jsxs("p", { className: "text-sm text-gray-600", children: [menu.items?.length || 0, " items \u2022 Select ", menu.quantity, " items"] })] }), _jsxs("div", { className: "flex gap-2", children: [!menu.approved && (_jsx("button", { onClick: () => menu.id && handleApprove(menu.id), className: "bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm transition", children: "Approve" })), _jsxs("button", { onClick: () => menu.id && handleDeleteMenu(menu.id), className: "bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm transition flex items-center gap-2", children: [_jsx(Trash2, { className: "w-4 h-4" }), "Delete"] })] })] }), menu.items && menu.items.length > 0 && (_jsxs("div", { className: "bg-gray-50 p-4 rounded-lg space-y-3", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("p", { className: "text-sm font-medium text-gray-700", children: ["Items (", getSelectedCount(menu.id, menu.items.length), " of ", menu.items.length, " selected):"] }), menu.timing && _jsx("span", { className: "text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded", children: menu.timing }), menu.gm_name && _jsxs("span", { className: "text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded", children: ["GM: ", menu.gm_name] })] }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3", children: menu.items.map((item, idx) => {
                                                const isSelected = selectedItems[menu.id]?.has(idx);
                                                return (_jsx("div", { onClick: () => toggleItemSelection(menu.id, idx), className: `p-3 rounded-lg border-2 cursor-pointer transition ${isSelected
                                                        ? 'bg-green-100 border-green-500 text-green-900'
                                                        : 'bg-white border-gray-200 text-gray-700 hover:border-gray-300'}`, children: _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("input", { type: "checkbox", checked: isSelected, onChange: () => toggleItemSelection(menu.id, idx), className: "w-4 h-4 cursor-pointer" }), _jsx("span", { className: "text-sm font-medium flex-1", children: item.name })] }) }, idx));
                                            }) })] }))] }, menu.id))) }))] }), _jsxs("div", { className: "bg-white rounded-lg p-6 shadow-sm border border-gray-200", children: [_jsx("h2", { className: "text-lg font-semibold text-gray-800 mb-4", children: "Category Summary" }), _jsx("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-4", children: categories.map((category) => {
