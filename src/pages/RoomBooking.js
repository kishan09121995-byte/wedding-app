...
import { Save, Bed, Download } from 'lucide-react';
...
export default function RoomBooking() {
    ...
    return (_jsxs("div", { className: "space-y-6", children: [_jsx("div", { className: "bg-white rounded-lg p-6 shadow-sm border border-gray-200", children: _jsxs("div", { className: "flex justify-between items-center", children: [_jsxs("div", { children: [_jsxs("h1", { className: "text-3xl font-bold text-gray-800 flex items-center gap-2", children: [_jsx(Bed, { className: "w-8 h-8 text-rose-gold" }), "Room Booking & Assignments"] }), _jsx("p", { className: "text-gray-600 text-sm mt-2", children: "Manage guest room assignments and check-in/out dates" })] }), _jsxs("div", { className: "flex gap-2", children: [_jsxs("button", { onClick: handleExportPDF, className: "flex items-center gap-2 px-4 py-2 bg-blue-100 text-blue-800 rounded-lg hover:bg-blue-200 transition-colors", children: [_jsx(Download, { className: "w-4 h-4" }), "Export PDF"] }), _jsx("button", { onClick: handleReset, disabled: saving, className: "flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-800 rounded-lg hover:bg-gray-200 transition-colors disabled:opacity-50", children: "Reset" }), _jsxs("button", { onClick: handleSaveAll, disabled: saving, className: "flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50", children: [_jsx(Save, { className: "w-4 h-4" }), saving ? 'Saving...' : 'Save All'] })] })] }) }), _jsxs("div", { className: "flex gap-2 flex-wrap", children: [_jsx("button", { onClick: () => setFilterHotel(null), className: `px-4 py-2 rounded-lg font-medium transition-colors ${filterHotel === null ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`, children: "All Hotels" }), hotels.map((hotel) => (_jsx("button", { onClick: () => setFilterHotel(hotel.id), className: `px-4 py-2 rounded-lg font-medium transition-colors ${filterHotel === hotel.id ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`, children: hotel.name }, hotel.id)))] }), _jsx("div", { className: "space-y-8", children: filteredHotels.map((hotelId) => {
